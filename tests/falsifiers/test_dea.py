...
from nn_repair.falsifiers import DifferentialEvolutionPGDAttack
...
class TestDifferentialEvolutionPGDAttack(unittest.TestCase):
    def test_acasxu_networks_inverse_property1(self):
        ...
        falsifier = DifferentialEvolutionPGDAttack(optimizer='Adam')
        ...
        for network, (counterexamples, _) in zip(networks, counterexamples_for_each_network):
            ...
            for counterexample in counterexamples:
                ...
                assert not prop.property_satisfied(counterexample.inputs_as_tensor().unsqueeze(0), network)[0]
    ...
    #     )
    #     falsifier = DifferentialEvolutionPGDAttack(
    #         optimizer='Adam', population_size=10, iterations=100,
    #     )
    #
    ...
    def test_two_neuron_test_net(self):
        ...
        for counterexample in counterexamples:
            assert all(lb <= x <= ub for x, (lb, ub) in zip(counterexample.inputs, input_bounds))
            assert not prop.property_satisfied(
                counterexample.inputs_as_tensor().unsqueeze(0), network
            )
